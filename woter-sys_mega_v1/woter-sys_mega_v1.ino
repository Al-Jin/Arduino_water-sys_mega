...
#define RELEY_In_Off 65           // Реле открытия подачи воды в баки к цифровому выходу D65, фиолетовый
  ...
  extern uint8_t Inconsola[];              // Новый шрифт
  extern uint8_t Arial_bold_16x16[];       // Новый шрифт
  extern uint8_t ArialNumFontPlus[];       // Новый шрифт
  extern unsigned int panda[0x1D88];       // Массив с картинкой Панда
...
int Kpoliv_cancel = 0;   // Коэффициент отмены полива после ввода
int Kpanda = 0;          // Коэффициент картинки Панда
...
unsigned long time12 = 0;   // Переменная для хранения точки отсчета времени удаления полива, если не нажата кнопка ввод
...
int polivat = 0;                 // Переменная для хранения сколько литров должно быть полито
uint32_t FlowSensor_puls = 0;     // считает сколько импульсов пришло с датчика потока

void setup()
{
  attachInterrupt(0, FlowSensor_Function, RISING); // Обработчик прерываний для датчика потока
...
if(buttonState5 == HIGH && Kpoliv == 0 && Kpoliv1 == 1){                   // Условие активации ввода полива
...
Serial.println(FlowSensor_puls);
...
if(millis() - time11 > 1000 && Kpoliv1 == 1){       // Условие вывода информации полив на экран раз в секунду - мигание
    ...
    myGLCD.print("L", 250, 150);                  //Выводим текст в кавычках
...
if(millis() - time12 > 120000 && Kpoliv1 == 1){       // Условие прекращения ввода полив, если не нажали ввод за 2 минуты
    ...
    myGLCD.drawBitmap(0, 107, 140, 54, panda, 2); // Выводим картинку Панда
//    myGLCD.setColor(0, 0, 0);                   // Задаем черный цвет прямоугольника
//    myGLCD.fillRect(0,126,270,210);             // Рисуем прямоугольник, для закрашивания раздела полива
  }
