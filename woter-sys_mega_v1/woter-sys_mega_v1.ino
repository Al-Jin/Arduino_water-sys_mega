...
#define RELEY_Ul_On 16            // Реле закрытия подачи воды на улицу к цифровому выходу D16
#define RELEY_Ul_Off 17           // Реле открытия подачи воды на улицу к цифровому выходу D17
#define RELEY_In_On 18            // Реле закрытия подачи воды в баки к цифровому выходу D18
#define RELEY_In_Off 19           // Реле открытия подачи воды в баки к цифровому выходу D19
...
if(buttonState2 == HIGH){                          // Условие отключения светодиода давления воды ноль на входе при нажатии кнопки
    ...
    myGLCD.print("          ", 10, 30);           //Выводим пробелы, чтобы убрать время с момента подачи воды
  }
...
if(p_in == 0 && millis() - time1 > 900000 && K1 == 0){       // Условие включения светодиода когда давление воды на входе равно 0 в течении 15 минут 900000
...
//    myGLCD.print("идет вода", 10, 40);     //Выводим текст в кавычках   
    myGLCD.print(":", 45, 30);             //Выводим текст в кавычках        
    if(h < 10){                            // Обнуление второго разряда часов
    myGLCD.printNumI(0, 10, 30);
    myGLCD.printNumI(h, 27, 30);
  }   else {
    myGLCD.printNumI(h, 10, 30);
  } if(m < 10){                      // Обнуление второго разряда минут
    myGLCD.printNumI(0, 61, 30);
    myGLCD.printNumI(m, 78, 30);
  }   else {
    myGLCD.printNumI(m, 61, 30);
  } if(s < 10){                      // Обнуление второго разряда минут
    myGLCD.setFont(Arial_bold_16x16);   //Задаем шрифт выводимого ниже на экран
    myGLCD.printNumI(0, 100, 35);
    myGLCD.printNumI(s, 117, 35);
  }   else {
    myGLCD.setFont(Arial_bold_16x16);   //Задаем шрифт выводимого ниже на экран
    myGLCD.printNumI(s, 100, 35);
  }} else {
    myGLCD.setFont(Arial_round_16x24);   //Задаем шрифт выводимого ниже на экран
    myGLCD.print("           ", 10, 30);           //Выводим пробелы, чтобы убрать время с момента подачи воды
  }
    ...
    myGLCD.setFont(Arial_round_16x24);   //Задаем шрифт выводимого ниже на экран
    myGLCD.print("p:", 10, 100);                    //Выводим текст в кавычках
    myGLCD.print("bar", 130, 100);                  //Выводим текст в кавычках
    myGLCD.printNumF(p_in, 2, 50, 100);             //Выводим значение давления на входе
