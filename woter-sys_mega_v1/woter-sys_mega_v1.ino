...
#define RELEY_In_On 16            // Реле закрытия подачи воды в баки к цифровому выходу D16
#define RELEY_In_Off 17           // Реле открытия подачи воды в баки к цифровому выходу D17
#define RELEY_Ul_On 18            // Реле закрытия подачи воды на улицу к цифровому выходу D16
#define RELEY_Ul_Off 19           // Реле открытия подачи воды на улицу к цифровому выходу D17
...
int KinOn = 0;           // Коэффициент открытия подачи воды в бак
...
if(buttonState1 == HIGH){                          // Условие отключения светодиода перелива при нажатии кнопки
    ...
    if (K1 == 0){
    KinOn = 0;                                    // Коэффициент открытия подачи воды в бак
    KinOff = 1;
    }
  }
...
if(buttonState2 == HIGH){                          // Условие отключения светодиода давления воды ноль на входе при нажатии кнопки
    ...
    myGLCD.print("      ", 10, 10);           //Выводим пробелы, чтобы убрать время с момента подачи воды
  }
...
//  digitalWrite(RELEY_In_Off, HIGH);
//}
//if ((K1 == 0 && millis() - time8 > 20000) || (K3 == 0 && millis() - time8 > 20000)) {  // Подача напряжения на электро кран для открытия подачи воды в бак 20 секунд
//  digitalWrite(RELEY_In_On, LOW);   Включение реле
//} else {
